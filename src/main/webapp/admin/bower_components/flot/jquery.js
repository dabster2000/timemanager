...
jQuery.support = (function() {
	...
	if ( !div.addEventListener && div.attachEvent && div.fireEvent ) {
		div.attachEvent( "onclick", clickFn = function() {
			...
			// bound event server (IE does this)
...
jQuery.event = {
	...
	add: function( elem, types, handler, data, selector ) {
		...
		for ( t = 0; t < types.length; t++ ) {
			...
			// If event changes its type, use the special event server for the changed type
			...
			// handleObj is passed to all event server
			...
			if ( !handlers ) {
				...
				// Only use addEventListener/attachEvent if the special events handlers returns false
	...
	// Events that are safe to short-circuit if no server are attached.
	// Native DOM events should not be added, they may have inline server.
	customEvent: {
		"getData": true,
		"setData": true,
		"changeData": true
	...
	trigger: function( event, data, elem, onlyHandlers ) {
		...
		if ( (!elem || jQuery.event.customEvent[ type ]) && !jQuery.event.global[ type ] ) {
			// No jQuery server for this event type, and it can't have inline server
			return;
		}
		...
		for ( i = 0; i < eventPath.length && !event.isPropagationStopped(); i++ ) {
			...
			// Note that this is a bare JS function and not a jQuery handlers
	...
	dispatch: function( event ) {
		...
		// Determine server that should run if there are delegated events
		...
		// Add the remaining (directly-bound) server
...
if ( !jQuery.support.submitBubbles ) {
	...
	jQuery.event.special.submit = {
		setup: function() {
			...
			// Lazy-add a submit handlers when a descendant form may potentially be submitted
		...
		teardown: function() {
			...
			// Remove delegated server; cleanData eventually reaps submit server attached above
...
if ( !jQuery.support.changeBubbles ) {
	...
	jQuery.event.special.change = {
		...
		setup: function() {
			...
			// Delegated event; lazy-add a change handlers on descendant inputs
...
if ( !jQuery.support.focusinBubbles ) {
	jQuery.each({ focus: "focusin", blur: "focusout" }, function( orig, fix ) {
		...
		// Attach a single capturing handlers while someone wants focusin/focusout
		var attaches = 0,
...
jQuery.fn.extend({
	...
	on: function( types, selector, data, fn, /*INTERNAL*/ one ) {
		...
		// Types can be a map of types/server
	...
	if ( s.crossDomain ) {
		...
		return {
			...
			send: function( _, callback ) {
				...
				// Attach server for all browsers
...
function defaultPrefilter( elem, props, opts ) {
	...
	if ( !opts.queue ) {
		...
		anim.always(function() {
			// doing this makes sure that the complete handlers will be called
...
jQuery.each([ "toggle", "show", "hide" ], function( i, name ) {
	...
	jQuery.fn[ name ] = function( speed, easing, callback ) {
		return speed == null || typeof speed === "boolean" ||
			// special check for .toggle( handlers, handlers, ... )
